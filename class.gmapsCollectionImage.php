<?php
/**
 * @file class.gmapsCollectionImage.php
 * @brief Contiene la definizione ed implementazione della classe gmapsCollectionImage.
 *
 * @version 1.0
...
/**
 * @ingroup gino-gmaps
 * @brief Classe per la gestione di immagini associate alle collezioni
 *
 * Campi:
 *
 * - **id**: identificativo punto di interesse
 * - **collection_id**: identificativo @ref gmapsCollection di appartenenza 
 ...
 *
 */
class gmapsCollectionImage extends propertyObject {

	private static $_tbl_image = 'gmaps_point_collection_image';
	...
	function __construct($id, $instance) {
		...
		$this->_data_www = CONTENT_WWW.'/gmaps/'.$this->_controller->instanceName().'/point/collections/images';
		$this->_data_dir = CONTENT_DIR.OS.'gmaps'.OS.$this->_controller->instanceName().OS.'point'.OS.'collections'.OS.'images';
	}
	 ...
	 * @return bool true
	 */
	public function setCollection_id($value) {
		
		if($this->_p['collection_id']!=$value && !in_array('collection_id', $this->_chgP)) $this->_chgP[] = 'collection_id';
		$this->_p['collection_id'] = $value;
		return true;
	...
	public function form($redirect) {
		...
		$buffer .= $gform->hidden('image_id', $this->id);
